...
func resetTree(repo string) {
	...
	_ = exec.Command("git", "-C", repo, "clean", "-fd", "-e", "arcdesk.toml").Run()
}
...
func renderDiff(r diffReport) string {
	...
	fmt.Fprintf(&b, "## 🤖 ARCDESK e2e — diff test-gen\n\n")
...
// repoRelFromModulePath turns "arcdesk/internal/agent/foo.go" into
