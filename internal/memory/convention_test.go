...
func TestDocPathPrefersExisting(t *testing.T) {
	proj := t.TempDir()
	// An existing ARCDESK.md should keep receiving notes (no split to AGENTS.md).
	if err := os.WriteFile(filepath.Join(proj, "ARCDESK.md"), []byte("x"), 0o644); err != nil {
		t.Fatal(err)
	}
	set := Load(Options{CWD: proj})
	if got := set.DocPath(ScopeProject); filepath.Base(got) != "ARCDESK.md" {
		t.Errorf("should append to the existing ARCDESK.md, got %s", got)
	}
