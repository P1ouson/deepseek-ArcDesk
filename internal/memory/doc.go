// Package memory implements ARCDESK's persistent memory. It mirrors Claude
// Code's two-layer model while honoring ARCDESK's cache-first architecture:
//
//   - Hierarchical doc memory: ARCDESK.md / AGENTS.md files discovered from the
...
const (
	ScopeUser     Scope = "user"     // ~/.config/arcdesk/ARCDESK.md
	ScopeAncestor Scope = "ancestor" // a ARCDESK.md above the project root
	ScopeProject  Scope = "project"  // ./ARCDESK.md (committed, shared)
	ScopeLocal    Scope = "local"    // ./ARCDESK.local.md (personal, git-ignored)
)

// docNames are the recognized memory filenames at each level, in load order.
// ARCDESK.md is ours; AGENTS.md and CLAUDE.md are the cross-tool conventions.
...
var docNames = []string{"ARCDESK.md", "AGENTS.md", "CLAUDE.md"}

// localNames are the personal, git-ignored overrides, highest precedence.
var localNames = []string{"arcdesk.local.md", "AGENTS.local.md", "CLAUDE.local.md"}
