...
func TestSaveImageDataURL(t *testing.T) {
	...
	if !strings.HasPrefix(got, ".arcdesk/attachments/clipboard-") || !strings.HasSuffix(got, ".png") {
...
func TestSaveAttachmentFile(t *testing.T) {
	...
	if !strings.HasPrefix(got, ".arcdesk/attachments/clipboard-") || !strings.HasSuffix(got, ".pdf") {
...
func TestImageDataURLRejectsOutsideAttachmentDir(t *testing.T) {
	...
	if _, err := ImageDataURL("../.arcdesk/attachments/x.png"); err == nil {
...
func TestImageDataURLRejectsSymlinkFile(t *testing.T) {
	...
	link := filepath.Join(".arcdesk", "attachments", "link.png")
...
func TestImageDataURLRejectsSymlinkAttachmentDir(t *testing.T) {
	t.Chdir(t.TempDir())
	if err := os.Mkdir(".arcdesk", 0o755); err != nil {
		t.Fatal(err)
	}
	if err := os.Mkdir("elsewhere", 0o755); err != nil {
		t.Fatal(err)
	}
	if err := os.Symlink("../elsewhere", filepath.Join(".arcdesk", "attachments")); err != nil {
		t.Skipf("symlink unsupported: %v", err)
	}
	if _, err := ImageDataURL(".arcdesk/attachments/x.png"); err == nil {
...
func TestImageDataURLRejectsSymlinkSubdirectory(t *testing.T) {
	...
	link := filepath.Join(".arcdesk", "attachments", "link")
