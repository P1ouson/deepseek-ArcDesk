...
func TestRunShellQuiet_NoEvents(t *testing.T) {
	var events []event.Event
	sink := event.FuncSink(func(e event.Event) {
		events = append(events, e)
	})
	...
	result := ctrl.RunShellQuiet("echo quiet")
	if result.Err != "" {
		t.Fatalf("unexpected error: %s", result.Err)
	}
	if !strings.Contains(result.Output, "quiet") {
		t.Fatalf("output should contain 'quiet', got: %q", result.Output)
	}
	if len(events) != 0 {
		t.Fatalf("RunShellQuiet should not emit events, got %d", len(events))
	}
}
