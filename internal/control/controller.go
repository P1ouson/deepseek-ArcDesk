...
func (c *Controller) execShell(ctx context.Context, command string, stream func(string)) shellExecResult {
	command = strings.TrimSpace(command)
	if command == "" {
		return shellExecResult{errMsg: i18n.M.ShellExecEmpty}
	}
	...
	var w io.Writer = &buf
	if stream != nil {
		w = io.MultiWriter(&buf, &shellWriter{emit: stream})
	}
	cmd.Stdout = w
	cmd.Stderr = w
	err := cmd.Run()
	...
	if ctx.Err() == context.DeadlineExceeded {
		return shellExecResult{out: out, errMsg: fmt.Sprintf(i18n.M.ShellExecTimeoutFmt, shellTimeout)}
	}
	if err != nil {
		return shellExecResult{out: out, errMsg: fmt.Sprintf(i18n.M.ShellExecFailedFmt, err)}
	}
	return shellExecResult{out: out}
}
...
func (c *Controller) RunShell(command string) {
	...
	c.runGuarded(func(ctx context.Context) error {
		...
		result := c.execShell(ctx, command, func(chunk string) {
			c.sink.Emit(event.Event{
		...
		c.sink.Emit(event.Event{
			...
			Tool: event.Tool{ID: id, Name: "bash", Output: result.out, Err: result.errMsg},
		})
		return nil
	})
}

// QuietShellResult is the outcome of RunShellQuiet — no events are emitted.
type QuietShellResult struct {
	Output string
	Err    string
}

// RunShellQuiet executes a shell command without streaming tool events to the
// UI transcript. Used for dock Git actions that should not appear as chat turns.
func (c *Controller) RunShellQuiet(command string) QuietShellResult {
	ctx, cancel := context.WithTimeout(context.Background(), shellTimeout)
	defer cancel()
	result := c.execShell(ctx, command, nil)
	return QuietShellResult{Output: result.out, Err: result.errMsg}
}
