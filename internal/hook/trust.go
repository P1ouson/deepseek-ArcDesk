...
// root. The trust flag lives in user-global state (~/.arcdesk/trust.json),
// NOT in the project file itself — an attacker controls the latter. Global
// hooks (~/.arcdesk/settings.json) are the user's own and always run.

// TrustFilename is the user-global trust store under ~/.arcdesk.
