...
func TestCollectReportRedactsSecrets(t *testing.T) {
	t.Setenv("arcdesk_TEST_SECRET", "sk-live-secret")
...
func TestCollectReportDoesNotRequireAPIKey(t *testing.T) {
	...
	if !strings.Contains(text, "arcdesk 1.2.3 doctor") {
...
func TestRenderTextSurfacesWarningsUpTop(t *testing.T) {
	text := RenderText(Report{Warnings: []string{"config ARCDESK.toml: parse boom"}})
