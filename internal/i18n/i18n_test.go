...
// $LC_ALL / $ARCDESK_LANG. Unknown locales return "" so DetectLanguage falls
...
// TestDetectLanguagePriority verifies override beats env and that ARCDESK_LANG
// beats LANG. With a clean env we fall back to English.
func TestDetectLanguagePriority(t *testing.T) {
	t.Setenv("arcdesk_LANG", "")
	t.Setenv("LC_ALL", "")
	...
	t.Setenv("arcdesk_LANG", "en")
	if got := DetectLanguage(""); got != "en" {
		t.Errorf("arcdesk_LANG=en overriding LANG=zh: got %q, want en", got)
	}
