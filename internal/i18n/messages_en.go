...
var English = Messages{
	...
	SetKeyHint:      "Set your API key, then run `ARCDESK chat`.",
	ConfigLabel:     "config",
	ModelsLabel:     "models",
	...
	StepScaffold:    "scaffold ARCDESK.toml",
	StepSetKey:      "set API key",

	InitHint:       "Project memory (AGENTS.md) is generated in-session: run `ARCDESK chat`, then `/init` — the model analyzes the codebase and writes it. For configuration, use `ARCDESK setup`.",
	StepSetKeyHint: "run `ARCDESK setup`, or export DEEPSEEK_API_KEY=…",
	StepChatDesc:   "interactive session",
	StepRunDesc:    "one-shot task",
	HelpFooter:     "arcdesk help · all commands",

	ChatTip:           "Context is kept across turns. Type 'exit' or Ctrl-D to quit.",
	TurnCancelled:     "cancelled — back to prompt",
	NoSessionToResume: "no saved session to resume — start a new one with `ARCDESK chat`",
	...
	OutputStyleHint:    "set agent.output_style in ARCDESK.toml to apply one (takes effect next session)",
	ThemeHeader:        "themes:",
	...
	SlashMCPNone:                 "no MCP servers configured — add a [[plugins]] entry in ARCDESK.toml",
	...
	AgentRequestCanceled:   "Request canceled.",
	AgentRequestTimeout:    "Request timed out. Please retry.",
	AgentNetworkError:      "Could not reach the API. Check the base URL, network, and proxy settings.",
	AgentStreamInterrupted: "Connection to the model was interrupted. Please retry.",
	...
	CmdKnowledge:    "show project knowledge",
	...
	CmdModelArg:     "<provider/model>",
	CmdEffortArg:    "auto|low|medium|high|max",
	CmdThemeArg:     "auto|light|dark|<style>",
	CmdLanguageArg:  "auto|en|zh",
	CmdAutoPlanArg:  "off|on",
	CmdHooksArg:     "list|trust",
	CmdSkillArg:     "show|enable|disable|new|paths",
	CmdMcpArg:       "add|show|tools|connect|remove",
	CmdRememberArg:  "<note>",
	CmdCompactArg:   "[focus]",
	...
	ArgLanguageAuto: "auto-detect from ARCDESK_LANG / locale",
	...
	ListHooksNone:       "hooks: none active — configure in .arcdesk/settings.json (project, after trust) or ~/.arcdesk/settings.json (global)",
	ListMcpHeader:       "mcp servers",
	ListMcpNone:         "mcp: no servers connected — add one in ARCDESK.toml ([[plugins]]) or a project .mcp.json",

	MemoryNone:             "memory: none — add with “/remember <note>” or create ARCDESK.md in the project root",
	...
	MissingKeyIntro:       "arcdesk.toml is ready — just an API key away.",
	WroteFileFmt:          "Wrote %s",
	SetupComplete:         "Setup complete.",
	SetupCancelled:        "setup cancelled.",
	TryHintFmt:            "Try: %s",
	NextHint:              "Next: set your API key (run `ARCDESK setup` or export DEEPSEEK_API_KEY=...), then run `ARCDESK run \"your task\"`.",
	...
	SkipStaleCustomEntryFmt:    "skipping stale %q entry from ARCDESK.toml (pointing at %s) — please remove it from [[providers]]",
	...
	UsageRunHint:              "usage: ARCDESK run [--model NAME] <task>",
	ErrorPrefix:               "error:",
	...
	ProviderErrAuth:                "Authentication failed (HTTP 401): your API key is missing, wrong, or expired. Check the key in .env or run `ARCDESK setup`.",
	...
	UsageBody: `ARCDESK — a config- and plugin-driven coding agent (multi-model)

Usage:
  ARCDESK chat [--model NAME] [-c|--continue] [--resume]   interactive session (multi-turn; -c resumes the latest, --resume picks one)
  ARCDESK run  [--model NAME] [--max-steps N] <task>   run one task and exit
  ARCDESK serve [--model NAME] [--addr HOST:PORT]      serve the session over HTTP+SSE (browser client at /)
  ARCDESK setup [path]                                 interactive config wizard; writes ARCDESK.toml (+ .env)
  ARCDESK config auto-plan [off|on]                    configure automatic plan mode
  ARCDESK mcp <add|remove|list>                        manage MCP servers in ARCDESK.toml
  ARCDESK doctor [--json]                              print redacted local diagnostics
  ARCDESK version
  ARCDESK help

Examples:
  ARCDESK chat
  ARCDESK chat --continue
...
Configuration:
  Resolution: flag > ./ARCDESK.toml > ~/.config/arcdesk/config.toml > built-in defaults
  Secrets come from the environment via api_key_env (e.g. DEEPSEEK_API_KEY).
  Run 'ARCDESK setup' to scaffold a config; see docs/SPEC.md.
`,
}
