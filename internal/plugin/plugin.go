// Package plugin is ARCDESK's MCP client. It connects to external MCP servers and
...
// protocolVersion is the MCP revision ARCDESK advertises during initialize.
...
type Spec struct {
	...
	// inherit ARCDESK's cwd (the default for user-configured plugins). It exists
...
// messages (notifications, requests like roots/list) are ignored — ARCDESK is a
...
func (c *Client) initialize(ctx context.Context) error {
	res, err := c.call(ctx, "initialize", map[string]any{
		...
		"clientInfo":      map[string]any{"name": "arcdesk", "version": "dev"},
	})
	if err != nil {
		return err
	}
...
type mcpTool struct {
	...
	// plugin that declares a tool read-only opts it into ARCDESK's parallel-dispatch
