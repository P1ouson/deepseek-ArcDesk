...
func TestStartAvailableKeepsGoodServers(t *testing.T) {
	...
	bad := Spec{Name: "bad", Command: "arcdesk-missing-mcp-binary"}
...
func TestStdioCommandNotFoundSuggestsPATHFix(t *testing.T) {
	...
	host, _ := StartAvailable(ctx, []Spec{{Name: "missing", Command: "arcdesk-missing-mcp-binary"}})
	defer host.Close()
	...
	for _, want := range []string{
		`command "arcdesk-missing-mcp-binary" not found on PATH`,
