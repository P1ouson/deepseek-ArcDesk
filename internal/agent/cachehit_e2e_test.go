...
const systemPrompt = "You are ARCDESK, a coding agent. Be concise and follow project conventions. " +
...
func TestReleaseCacheHitGuard(t *testing.T) {
	if os.Getenv("arcdesk_RELEASE_CACHE_GUARD") == "" {
		t.Skip("set ARCDESK_RELEASE_CACHE_GUARD=1 to run the release cache guard")
	}

	threshold := envInt("arcdesk_CACHE_GUARD_THRESHOLD", 90)
	maxLowCases := envInt("arcdesk_CACHE_GUARD_MAX_LOW_CASES", 1)
	...
	if len(lows) > maxLowCases {
		...
		if os.Getenv("arcdesk_CACHE_GUARD_STRICT") != "" {
			t.Fatal(msg)
		}
	}
}
