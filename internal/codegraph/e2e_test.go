...
// ARCDESK_CODEGRAPH_E2E so the normal `go test ./...` skips it (no network, no
...
//
//	ARCDESK_CODEGRAPH_E2E=1 ARCDESK_CODEGRAPH_BIN=/path/to/codegraph \
//	  go test ./internal/codegraph/ -run E2E -v -count=1
//
// With ARCDESK_CODEGRAPH_BIN unset it falls back to Resolve("") (bundle / PATH).
func TestE2ECodegraphMCP(t *testing.T) {
	if os.Getenv("arcdesk_CODEGRAPH_E2E") == "" {
		t.Skip("set ARCDESK_CODEGRAPH_E2E=1 to run the CodeGraph MCP end-to-end test")
	}
	bin := os.Getenv("arcdesk_CODEGRAPH_BIN")
	if bin == "" {
		var ok bool
		if bin, ok = Resolve(""); !ok {
			t.Fatal("arcdesk_CODEGRAPH_E2E is set but no codegraph binary found — set ARCDESK_CODEGRAPH_BIN to the launcher path")
		}
	}
