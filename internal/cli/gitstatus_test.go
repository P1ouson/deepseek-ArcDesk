...
func TestGitStatusRenderWithinCompactsRepoBeforeBranch(t *testing.T) {
	status := gitStatus{Repo: "VeryLongDeepSeekARCDESKWorkspace", Branch: "codex/cli-tui-status-row"}

	full := ansi.Strip(status.RenderWithin(80, statusAutoColor))
	if full != "VeryLongDeepSeekARCDESKWorkspace@codex/cli-tui-status-row" {
...
func TestGitStatusRenderWithinKeepsDirtySuffix(t *testing.T) {
	status := gitStatus{
		Repo:      "VeryLongDeepSeekARCDESKWorkspace",
...
func TestLoadGitStatus(t *testing.T) {
	...
	runGitForTest(t, root, "config", "user.email", "arcdesk@example.invalid")
	runGitForTest(t, root, "config", "user.name", "arcdesk Test")
