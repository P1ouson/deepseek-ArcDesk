...
func renderHooks(width int, hooks []hook.ResolvedHook, trusted bool, projectDefines bool) string {
	...
	case trusted:
		b.WriteString(viewHint(viewCompactText("project trusted · config: project .arcdesk/settings.json + global ~/.arcdesk/settings.json", viewBudget(width, 2))))
	default:
		b.WriteString(viewHint(viewCompactText("project not trusted · config: project .arcdesk/settings.json + global ~/.arcdesk/settings.json", viewBudget(width, 2))))
	}
