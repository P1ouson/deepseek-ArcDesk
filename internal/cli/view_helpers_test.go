...
func TestRenderMemoryGroupsDocsAndStore(t *testing.T) {
	...
	got := renderMemory(width, &memory.Set{
		Docs:  []memory.Source{{Path: "/Users/me/project/ARCDESK.md", Scope: memory.ScopeProject}},
		Store: store,
		Index: store.Index(),
	})
	for _, want := range []string{"memory", "docs", "(project)", "ARCDESK.md", "saved memories", "saved-fact", "Saved Fact", "doc edits apply next session"} {
		if !strings.Contains(got, want) {
...
func TestRenderSkillPathsStaysWithinWidth(t *testing.T) {
	...
	got := renderSkillPaths(width, []skill.Root{{
		Dir:      "/Users/me/projects/really/deep/path/to/.arcdesk/skills",
