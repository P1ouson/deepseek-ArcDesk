    ...
    <div className={`tool tool--${item.status}${diffPanels.length ? " tool--diff" : ""} ${quiet ? "tool--quiet" : ""}`}>
      ...
      {shellPreview && open && (
        <div className="tool__body">
          <CodeViewer flat value={showAll ? shellOutput! : shellPreview.preview} maxHeight={showAll ? 480 : 260} />
      ...
      {!shellPreview && hasBody && open && (
        <div className="tool__body">
          {item.args && <CodeViewer flat value={pretty(item.args)} language="json" maxHeight={180} />}
          {item.output && (
            <>
              <CodeViewer flat value={item.output} maxHeight={280} />
              {item.truncated && <div className="tool__note">{t("tool.truncated")}</div>}
            </>
          )}
        </div>
