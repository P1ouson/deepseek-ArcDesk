...
import (
	...
	"arcdesk/desktop/internal/update"
)

// platforms are the manifest keys we publish. A built artifact is matched to a key
// by substring (file names embed the key, e.g. ARCDESK-darwin-arm64.zip), so the
...
// (ARCDESK.key) and the public key (ARCDESK.pub) into dir. The password comes
...
func genKey(dir string) error {
	...
	keyPath := filepath.Join(dir, "arcdesk.key")
	pubPath := filepath.Join(dir, "arcdesk.pub")
...
func signFiles(files []string) error {
	...
	for _, f := range files {
		...
		sig := minisign.SignWithComments(priv, data,
			"file:"+filepath.Base(f), "arcdesk desktop release")
		out := f + ".minisig"
