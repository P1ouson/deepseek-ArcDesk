...
| | |
|---|---|
| **桌面优先** | **Windows 安装包已发布**；macOS / Linux 安装包随 Release 提供（见下表） |
...
| 平台 | 安装包 | 说明 |
|------|------|------|
| **Windows** | [`.exe` 安装向导](https://github.com/P1ouson/ArcDesk/releases/latest/download/arcdesk-desktop-windows-amd64-installer.exe) | 约 10 MB，可选安装目录，无需管理员 |
| **macOS** | [`.dmg`](https://github.com/P1ouson/ArcDesk/releases/latest/download/arcdesk-desktop-darwin-universal.dmg) | 拖入「应用程序」；首次运行若被拦截见 [故障排查](#故障排查) |
| **Linux** | [`.tar.gz` 安装包](https://github.com/P1ouson/ArcDesk/releases/latest/download/arcdesk-desktop-linux-amd64-installer.tar.gz) | 解压后运行 `./install.sh`（安装到 `~/.local`） |

1. 从表格或 **[GitHub Releases](https://github.com/P1ouson/ArcDesk/releases)** 下载对应平台的**安装包**（不是源码 zip）。
...
本仓库**暂未发布 npm 包**。CLI 与桌面共用同一 Go 内核，可在本地构建：

```sh
make build          # -> bin/ARCDESK(.exe) 或 bin/arcdesk(.exe)
./bin/arcdesk chat  # 交互式终端
```

配置见 [`arcdesk.toml`](./ARCDESK.example.toml)（项目）与 `~/.config/arcdesk/config.toml`（用户）。
...
| 现象 | 处理 |
|------|------|
| macOS「应用已损坏」 | 未签名构建： `xattr -dr com.apple.quarantine /Applications/ArcDesk.app` |
...
更多：[GitHub Issues](https://github.com/P1ouson/ArcDesk/issues)
...
| **工程化** | GitHub Actions 三平台安装包构建；含桌面安全回归测试 |
...
Go agent 内核参考 [**Reasonix**](https://github.com/esengine/DeepSeek-Reasonix) 及其贡献者（详见 [渊源 — 与 Reasonix 的关系](#渊源--与-reasonix-的关系)）。
