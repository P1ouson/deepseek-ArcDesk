...
> 安装包尚未代码签名（开源项目常见情况）。macOS / Windows 首次运行可能被系统拦截 — 见 [故障排查](#故障排查)。Linux 从应用菜单启动即可，不必改 PATH。
...
| Windows SmartScreen | 未签名安装包的正常提示 → *更多信息 → 仍要运行*（安装程序会顺带装 WebView2） |
| Windows 空白窗口 | 手动安装 [WebView2](https://developer.microsoft.com/microsoft-edge/webview2/) |
| Linux 终端找不到命令 | 把 `~/.local/bin` 加入 PATH（从应用菜单启动不需要） |
