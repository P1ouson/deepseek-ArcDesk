...
| 亮点 | 做什么 | 说明 |
|------|--------|------|
| **代码工作区** | 项目级 Agent 会话 | 快捷卡片（项目导览 / 最近变更 / 定位逻辑）一键发起任务；支持 **auto / plan / yolo** 三档执行策略；底部实时显示上下文占用、压缩率与 API 用量 |
...
1. 导入项目工作区
2. 选择模型与推理强度（Settings → Models）
3. 在**代码模式**下发任务；Agent 读取文件、调用工具
4. 在 Diff 面板审阅变更；批准或拒绝写操作
5. 在**扩展**页管理 Skills / MCP；首次加载项目 MCP 需显式信任
6. 可选：Plan 模式分步执行；YOLO 模式需先配置项目沙盒
